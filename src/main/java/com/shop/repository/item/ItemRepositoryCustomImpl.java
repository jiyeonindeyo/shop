...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {
    ...
    // 조건1
    private BooleanExpression regDtsAfter(String searchDateType) { // BooleanExpression : where절 조건으로 들어가는 타입
        // searchDateType화면 -> "all", "1d", "1w", "1m", "6m"
    ...
    public Page<Item> getAdminPage(ItemSearchDto itemSearchDto, Pageable pageable) {
        ...
        // 조건2 - searchSellStatus에 따라 상품 판매 상태(SELL, SOLD_OUT) 설정
        // 조건3 - searchBy + searchQuery에 따라 검색 키워드 설정
        // ━▶ item id를 기준으로 내림차순 정렬(최신순), pageable 기준에 따른 페이징된 결과 반환
        // SELECT * FROM item WHERE 조건1 AND 조건2 AND 조건3 ORDER BY item_id DESC LIMIT, OFFSET ... ;
        // Page(인터페이스) >> PageImpl(구현체) >> content(LIst<T>), totalCount(총 페이지 수), number(페이지 번호)
        List<Item> content = jpaQueryFactory.selectFrom(item) //selectFrom(Q클래스)
                                            // where(조건1, 조건2, 조건3) -> null이 들어가면 해당 조건 무시(생략)
        ...
        // SELECT count(*) FROM item WHERE 조건1 AND 조건2 AND 조건3
        Long totalCount = jpaQueryFactory.select(Wildcard.count)
                                         ...
                                         .where(regDtsAfter(itemSearchDto.getSearchDateType()),
                                                ...
                                                searchByLike(itemSearchDto.getSearchBy(), itemSearchDto.getSearchQuery()))
                                         .fetchOne();
        // count 쿼리된 totalCount 결과는 null이 될 수 있음 => null을 안전하게 다루기 위해 Optional 타입으로 한번 랩핑한다(감싼다)
        // Optional : null일때 아닐때를 분개시켜주는 역할 -> null에 대한 안정성 확보
        Optional<Long> total = Optional.ofNullable(totalCount); // Optional.ofNullable : null일 수도 있음
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
        // dto 매핑까지 끝내고 반환받는것 -> 프로젝션
        List<MainItemDto> content = jpaQueryFactory.select(Projections.fields(MainItemDto.class,
                                                                              item.id,
                                                                              item.itemNm,
                                                                              item.itemDetail,
                                                                              item.price,
